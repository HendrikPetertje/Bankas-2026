...
The dock SHALL visually indicate which slide is currently active. The active slide button SHALL render a dedicated active-state image (`activeSrc`) instead of the default image. Hover effects (`hover:brightness-90`) SHALL apply to both active and inactive buttons. No CSS brightness/contrast filter toggle is used for active state.
