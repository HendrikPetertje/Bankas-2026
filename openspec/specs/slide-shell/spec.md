...
The `Slide` component SHALL accept a `dipFrom` prop (CSS color string). On initial render, a full-screen overlay (`fixed inset-0 z-50 pointer-events-none`) with that color SHALL be visible and SHALL fade to transparent over 2 seconds.
...
The `Slide` component SHALL accept a `dipTo` prop (CSS color string) and a `dipToActive` boolean prop. The overlay SHALL only become opaque when `dipToActive` is `true`. The `dipToActive` prop SHALL be managed exclusively by App.tsx — set to `true` only on the OLD (current) slide before the swap, and set back to `false` before mounting the new slide.
...
- **WHEN** App.tsx sets `dipToActive=true` on the current slide
- **THEN** the dipTo overlay transitions from transparent to the `dipTo` color over 2 seconds

#### Scenario: New slide mounts without dipTo flash

- **WHEN** a new slide mounts after a swap
- **THEN** its dipTo overlay is not active (dipToActive is false), so no flash occurs
